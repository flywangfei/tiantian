...
<TABLE cellSpacing=0 cellPadding=0 width=680>
  <TBODY>
  <TR>
    <TD height=80 align=center><INPUT class=button_36 <?php if($dduser['name']==''){?> onclick="alert('登陆后才能评论！');window.location='<?=SITEURL.'/'.u('user','login')?>&url='+encodeURIComponent(window.location.href)" <?php }else{?>onclick="saveComment()"<?php }?> name=Submit2 value="评 论" type=submit></TD></TR></TBODY></TABLE></DIV></DIV>
</div>
