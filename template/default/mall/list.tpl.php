...
<!--购物返现开始-->
<div class="gouwufanxian">
<div class="gwbiaoti"><h3>返现商城</h3> </div>
...
<input class="fusearchbt" name="" type="submit" value="" />
</form>
...
<li>
<div class="fuxuanting"> 
  <div class="fuxt01"><div class="fuxt01b"><img alt="<?=$row['title']?>" src="<?=$row['img']?>" /></div><div class="fuxt01a">返 <span><?=$row['fan']?></span></div></div>
    <div class="fuxt02">
      <ul>
        <li><a href="<?=u('mall','view',array('id'=>$row['id']))?>"><img alt="返现详情" src="<?=TPLURL?>/images/fx01.png" /></a></li>
        <li><a target="_blank" href="<?=u('jump','mall',array('mid'=>$row['id']))?>"><img alt="直接购买" src="<?=TPLURL?>/images/fx02.png" /></a></li>
      </ul>
    </div>
    <div class="fuxt03"><?=utf_substr($row['des'],46)?>...</div>
    <div class="fuxt04">评分:<?=$row['score']?>&nbsp;&nbsp;&nbsp;[<?=$row['pjnum']?>条评论]</div>
    </div>
<div class="pailie"><a href="<?=u('mall','view',array('id'=>$row['id']))?>"><img alt="<?=$row['title']?>" src="<?=$row['img']?>" /></a>  <p><?=$row['title']?> | 最高返<span id="fontk"><?=$row['fan']?></span></p></div>
</li>
<?php }?>
  </ul>
