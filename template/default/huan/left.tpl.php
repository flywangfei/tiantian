<div id="apDiv9">
    ...
    <div>积分：<span style=" color:#B1319E; font-size:14px"><?=$dduser['live_jifen']?></span>&nbsp;点</div>
    <div>余额：<span style=" color:#B1319E; font-size:14px"><?=$dduser['live_money']?></span>&nbsp;元</div>
    </div>
    </div>
    <?php }else{?>
      <div id="apDiv22"></div>
      <div id="apDiv23"><a onclick="window.location='<?=SITEURL.'/'.u('user','login')?>&url='+encodeURIComponent(window.location.href)" style="cursor:pointer"><img alt="登陆" src="<?=TPLURL?>/images/fqw_dl.gif" width="171" height="35" /></a></div>
    <?php }?>
    </div>
