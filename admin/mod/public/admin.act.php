...
switch (MOD) {
	...
	case 'mall':
		...
		if ($_POST['sub'] != '' && ACT == 'addedi') {
			$array=array('yiqifaid','duomaiid','chanetid','chanet_draftid','edate','merchantId');
	...
	case 'slides':
		...
		$category=array(1=>'网站首页','淘宝首页','拍拍首页');
	...
	case 'tixian':
	    $status_arr=array(0=>'<span style="color:#ff3300">提现待审核</span>',1=>'<span style="color:#009900">提现成功</span>',2=>'<span style="color:#333333">提现失败</span>');
	break;
	...
	case 'sms';
		include(DDROOT.'/comm/sms.class.php');
		if(ACT=='set'){
			if(!isset($_POST['sub'])){
				if($webset['sms']['name']!='' && $webset['sms']['pwd']!=''){
					$row=sms_get($webset['sms']['name'],$webset['sms']['pwd']);
					if(isset($row['Code']) && $row['Code']>0){
						$re='<b style="color:red">'.$sms_error[$row['Code']].'</b>';
					}
					else{
						$re='短信剩余<b style="color:red">'.$row['Balance'].'</b>条';
					}	
				}
				$msgset_title=$duoduo->select_2_field('msgset','id,title','1=1');
			}
			if(isset($_POST['sub'])){
				if($_POST['sms']['name']!='' && $_POST['sms']['pwd']!=''){
					if($_POST['sms']['pwd']==DEFAULTPWD){
						$_POST['sms']['pwd']=$webset['sms']['pwd'];
					}
					if($_POST['sms']['newpwd']==DEFAULTPWD){
						$_POST['sms']['newpwd']=$webset['sms']['pwd'];
					}
					
					$row=sms_set($_POST['sms']['name'],$_POST['sms']['pwd'],$_POST['sms']['newpwd']);
					if(isset($row['Code']) && $row['Code']!=0){
						jump(-1,$sms_error[$row['Code']]);
					}
					if(empty($row)){
						jump(-1,'修改密码发生未知错误');
					}
			...
			if(isset($_POST['test']) && $_POST['test']==1){
				$name=$_POST['name'];
				$pwd=$_POST['pwd']==DEFAULTPWD?$webset['sms']['pwd']:$_POST['pwd'];
				$mobile=$_POST['mobile'];
				$row=sms_send($name,$pwd,$mobile,'测试短信');
				if(isset($row['Code']) && $row['Code']==0){
					echo 1;
				}
				else{
