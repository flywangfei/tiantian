...
if ($_POST['sub'] != '' || $_GET['sub'] != '') {
	...
	if ($do == 4) { //多麦
	    ...
	    for($i=$fromnum;$i<$maxnum;$i++){
			...
			$mall_order = $duoduo->select("mall_order", "*", 'unique_id="' . $unique_id . '" and lm=4');
	...
	if ($do == 2) { //领克特
		...
		for($i=$fromnum;$i<$maxnum;$i++){
			...
			$mall_order = $duoduo->select("mall_order", "*", 'unique_id="' . $unique_id . '"  and lm=2');
			if ($mall_order['id'] == '') { //交易不存在
				$field_arr = array (
					'adid' => $merchant_id,
					'lm' => 2,
	...
	if ($do == 3) { //亿起发
	    ...
	    for($i=$fromnum;$i<$maxnum;$i++){
			...
			$mall_order = $duoduo->select("mall_order", "*", 'unique_id="' . $unique_id . '" and lm=3');
			if ($mall_order['id'] == '') { //交易不存在
				$field_arr = array (
					'adid' => $adid,
					'lm' => 3,
	...
	if ($do == 1) { //成果
		...
		if (get_cookie('chanet_j_id') == '') {
		    $url = 'http://www.chanet.com.cn/rest/as/get_as_ec_list.cgi?user_name=' . $webset['chanet']['name'] . '&password=' . $webset['chanet']['pwd'] . '&token=' . $webset['chanet']['key'] . '&as_id=' . $webset['chanet']['wzid'] . '&start_time=' . $eday . '&end_time=' . $eday . '&confirm_status=all&userinfo=yes';
		    $collect->get($url);
			$c = $collect->val;
		...
		if($c>0){
			...
			for($i=$fromnum;$i<$maxnum;$i++){
			    ...
			    $mall_order = $duoduo->select("mall_order", "*", 'unique_id="' . $unique_id . '" and lm=1'); //用订单编号和商品编号查
			    if ($mall_order['id'] == '') { //交易不存在
				    $field_arr = array (
					    'adid' => $adid,
					    'lm' => 1,
